...
In order to reproduce the code, please follow the instructions below:
  ...
  6. If you are using a dataset originally designed for YoloV3, you need to convert it to YoloV5 format. To do so, run:
  ```bash
  python utils/upgrade_v3dataset.py
  ```

  7. Now set the environment variable ORGANTRANS_DATASET pointing to the path of dataset.yaml file included included in the dataset folder. If you just upgraded to yoloV5 format, make sure to use the newly created file (with a suffix _v5).
  ```bash	
  export ORGANTRANS_DATASET=/path/to/dataset.yaml
  ```

  8. Run the training script (for additional arguments, run `python train.py --help`):
  ```bash
  python train.py 
    --data %ORGANTRANS_DATASET%
    --weights yolov5s.pt
    --batch-size 16
    --epochs 30
    --seed 42
  ...
  8. Your model will be automatically saved to `runs/{experiment_name}/weights/best.pt`. Set a new environment variable pointing to the path of the weights file generated by the training script:
  ```bash
  export ORGANTRANS_MODEL=/path/to/weights.pt
  ```

  9. Run the test script (for additional arguments, run `python val.py --help`):
    ```bash
    python val.py 
        --weights %ORGANTRANS_MODEL%
        --task test 
    ```
